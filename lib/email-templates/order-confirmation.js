export function generateOrderConfirmationEmail({ 
  ...
  // Update the validateImageUrl function
  const validateImageUrl = (url) => {
    if (!url) return `${process.env.NEXT_PUBLIC_BASE_URL}${DEFAULT_IMAGE}`;
    try {
      // Handle both relative and absolute URLs
      const validatedUrl = url.startsWith('http') ? url : `${process.env.NEXT_PUBLIC_BASE_URL}${url}`;
      
      // Log the URL transformation
      console.log('🖼️ Validating image URL:', {
        original: url,
        transformed: validatedUrl
      });
