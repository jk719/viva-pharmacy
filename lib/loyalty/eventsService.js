...
class LoyaltyEventsService {
  constructor() {
    ...
    this.minEmissionInterval = 100; // Minimum time between emissions in ms
  }
  ...
  async processEvent(userId, event) {
    ...
    try {
      ...
      switch (event.type) {
        case 'ORDER_COMPLETE':
          // Emit a single consolidated loyalty update
          eventEmitter.emit(Events.LOYALTY_UPDATE, {
            ...event.data,
            type: Events.LOYALTY_UPDATE,
            retryCount,
            isComplete: true,
            timestamp: new Date().toISOString()
          });
