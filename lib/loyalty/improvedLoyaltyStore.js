...
import { calculateProgressToNextTier, calculateUserTier, getTierMultiplier } from './loyaltyCalculator';
  ...
  persist(
    (set, get) => ({
      ...
      fetchUserData: async (forceRefresh = false) => {
        ...
        const cacheExpired = cacheAge > TIMING.LOYALTY.CACHE_EXPIRE; // 5 minutes
      ...
      forceRefresh: async () => {
        console.log('[LoyaltyStore] Force refreshing loyalty data');
        return await get().fetchUserData(true);
      },

      // Phase 3: Cleanup old surprises
      cleanupOldSurprises: () => {
        const { surpriseHistory } = get();
        const cutoffTime = Date.now() - TIMING.CACHE.DAY; // 24 hours
        
        const recentSurprises = surpriseHistory.filter(
          surprise => new Date(surprise.timestamp).getTime() > cutoffTime
        );
        
        if (recentSurprises.length !== surpriseHistory.length) {
          set({ surpriseHistory: recentSurprises });
        }
      }
    }),
