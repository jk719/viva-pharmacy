...
export function getCloudinaryUrl(product) {
  try {
    ...
    // Handle Amazon image URLs properly - prioritize these if available
    // This is likely the main issue - we need to properly handle external image URLs
    if (product.imageUrl && (
      product.imageUrl.startsWith('http://') || 
      product.imageUrl.startsWith('https://')
    )) {
      // If the URL includes "amazon" or "media-amazon", it's an Amazon image
      if (product.imageUrl.includes('amazon.com') || product.imageUrl.includes('media-amazon')) {
        // For Amazon images, replace http:// with https:// for security
        const secureUrl = product.imageUrl.replace('http://', 'https://');
        return secureUrl;
      }
      
      // For other external URLs, use them directly if they're not Cloudinary
    ...
    // Try to construct URL from product name as a last resort
    if (product.name) {
