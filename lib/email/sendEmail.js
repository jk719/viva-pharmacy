...
import { generateDeliveryConfirmationEmail } from '../email-templates/delivery-confirmation.js';
...
export async function sendOrderEmail({ to, subject, content }) {
    return sendEmail({
        to,
        subject,
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="color: #666; font-size: 14px;">
                    Thank you for shopping with Viva Pharmacy
                </p>
            </div>
        `,
        headers: {
...
export async function sendDeliveryConfirmation({ to, deliveryDetails }) {
    try {
        console.log('🚀 Starting delivery confirmation email send process...');
        const emailContent = generateDeliveryConfirmationEmail(deliveryDetails);
        ...
        console.error('❌ Delivery confirmation email send error:', error);
        throw error;
    }
}
