...
// Conditional import for analytics - only import if package is available
let Analytics = null;
try {
  Analytics = require('@segment/analytics-node').Analytics;
} catch (error) {
  console.warn('Segment Analytics not available:', error.message);
}
...
const analytics = (process.env.SEGMENT_WRITE_KEY && Analytics) 
