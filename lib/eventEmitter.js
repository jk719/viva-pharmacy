...
const eventTracking = {
  ...
  canEmitEvent(type) {
    // SPECIAL CASE: Always allow loyalty updates to emit with no restrictions
    // This is critical for proper animation in the checkout flow
    if (type === Events.LOYALTY_UPDATE) {
      return true; // Always allow loyalty updates to pass through
    }
...
const safeEmit = (type, data) => {
  try {
    ...
    // Special bypass for critical events with force flags
    if ((type === Events.LOYALTY_UPDATE && data?.forceAnimation) || type === Events.PING) {
      // Special handling for critical events with bypass flag
      console.log(`⭐ Processing ${type} event with bypass`);
