...
eventEmitter.on(Events.PAYMENT_COMPLETED, (data) => {
  // Skip if already processed by the payment service
  if (data && data.processedBy === 'paymentService') {
    return;
  }
  
  // Skip if we're on a non-checkout page like giveaway
  if (typeof window !== 'undefined' && window.location.pathname.includes('/giveaway')) {
    console.log('Ignoring payment completed event on giveaway page');
    return;
  }
  
  // Forward to centralized payment service
  paymentService.handlePaymentCompleted(data, 'eventEmitter');
});
