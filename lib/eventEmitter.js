...
export const Events = {
  ...
  // Animation related events - marked as deprecated, don't use these
  LOYALTY_UPDATE: 'loyalty_update_deprecated',
  LOYALTY_ANIMATION_COMPLETE: 'loyalty_animation_complete_deprecated',
  PROGRESS_BAR_ANIMATION_COMPLETE: 'progress_bar_animation_complete_deprecated'
};
...
const eventTracking = {
  ...
  canEmitEvent(type) {
    // Normal event handling - no special cases for animation events
...
const safeEmit = (type, data) => {
  try {
    ...
    // Special bypass for critical events only
    if (type === Events.PING) {
